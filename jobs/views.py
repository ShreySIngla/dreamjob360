...
from .globalfunctions import paginate_results
...
def latest_blogs(request):
    ...
    all_blogs = blog.objects.all().order_by('-created_on')

    blogs_per_page = 2

    latest_blogs = paginate_results(request, all_blogs, blogs_per_page)
...
def job_list(request, *args, **kwargs):
    ...
    if not user.is_authenticated:
        ...
        applied_jobs = AppliedJob.objects.filter(jobseeker=user)
        jobs = job.objects.all().order_by('-created_on')
        all_jobs =jobs.exclude(id__in=applied_jobs.values_list('job_id', flat=True))

    jobs_per_page = 3
    job_list = paginate_results(request, all_jobs, jobs_per_page)
...
def index(request, *args, **kwargs):
    ...
    if not user.is_authenticated:
        ...
        applied_jobs = AppliedJob.objects.filter(jobseeker=user)
        all_jobs = job.objects.all().order_by('-created_on')
        jobs =all_jobs.exclude(id__in=applied_jobs.values_list('job_id', flat=True))
...
def profile(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        details=userdetails.objects.create(jobseeker=user,age=age,dob=dob,gender=gender,experience=experience,address=address,resume=resume)
...
def search_list( request, *args, **kwargs):
    ...
    user = request.user
    if not user.is_authenticated:
        posts = job.objects.filter(Q(title__icontains=keywords) |  Q(salary__icontains=keywords) |  Q(location__icontains=keywords) |  Q(company__icontains=keywords) ).order_by('-created_on')
    
    else:
        applied_jobs = AppliedJob.objects.filter(jobseeker=user)
        all_posts = job.objects.filter(Q(title__icontains=keywords) |  Q(salary__icontains=keywords) |  Q(location__icontains=keywords) |  Q(company__icontains=keywords) ).order_by('-created_on')
        posts =all_posts.exclude(id__in=applied_jobs.values_list('job_id', flat=True))
      
    
    # posts = job.objects.filter(Q(title__icontains=keywords) |  Q(salary__icontains=keywords) |  Q(location__icontains=keywords) |  Q(company__icontains=keywords) ).order_by('-created_on')

    jobs_per_page = 5
    searched_jobs = paginate_results(request, posts, jobs_per_page)
    print(keywords)
...
class JobDetailView(LoginRequiredMixin, generic.DetailView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        context = {
            'job': job,
            'has_applied': True,  # Set has_applied to True after applying
        }
        return render(request, 'job-detail.html', context)
...
@login_required(login_url='/login/')
def appliedjobs(request, *args, **kwargs):
    user = request.user
    ...
    if not user.is_authenticated:
        ...
        all_jobs = AppliedJob.objects.filter(jobseeker=user).order_by('-id')

        jobs_per_page = 3

        job_list = paginate_results(request, all_jobs, jobs_per_page)

        return render(request, 'appliedjobs.html', {'job_list': job_list})
